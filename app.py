...
if st.session_state.conectado:
    ...
    if st.session_state.monitoreando:
        ...
        # Verificar si hay alguna señal activa que ya expiró (5 min después de entrada)
        ...
        for asset, senal in activas.items():
            if senal.get('activa'):
                ...
                if entrada_completa < now:
                    entrada_completa += timedelta(days=1)  # por si es de otro día
        ...
        # Si hay alguna señal activa, no generamos nuevas hasta que pasen 5 min desde la última entrada
        ...
        if st.session_state.activos_seleccionados:
            for asset in st.session_state.activos_seleccionados:
                # Si ya hay una señal activa para este activo, no evaluar (ya está en curso)
            ...
            # Si no hubo señales, reevaluar la fuerza de los activos periódicamente (cada ciclo)
            # Podríamos también refrescar la lista de activos cada cierto tiempo
            time.sleep(pausa_entre_ciclos)
            # Actualizar activos (opcional)
            if st.session_state.indice_ronda % 10 == 0:
                # Cada 10 ciclos, refrescar la lista de activos fuertes
                activos_totales = obtener_activos_abiertos(st.session_state.api, tipo_mercado)
                if activos_totales:
